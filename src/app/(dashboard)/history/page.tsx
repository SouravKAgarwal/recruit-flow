...
import { CheckCircle, XCircle, Clock, Mail, Building2, FileText } from "lucide-react";
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800 }}>Email History</h1>
      ...
      {logs.length === 0 && (
        <div className="glass" style={{ padding: 48, textAlign: "center" }}>
          <Mail size={40} style={{ color: "var(--color-text-dim)", margin: "0 auto 16px" }} />
        ...
        {logs.map((log, idx) => {
          ...
          return (
            <div key={log.id} style={{ display: "flex", gap: 16, position: "relative" }}>
              {/* Timeline line */}
              {idx < logs.length - 1 && (
                <div style={{ position: "absolute", left: 19, top: 40, bottom: 0, width: 1, background: "var(--color-border)" }} />
              )}

              {/* Status dot */}
              <div style={{ flexShrink: 0, width: 40, display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 14 }}>
                <div style={{
              ...
              {/* Content */}
              <div className="glass" style={{ flex: 1, padding: "14px 18px", marginBottom: 10, marginTop: 4 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {log.subject && (
                      <p style={{ fontSize: 13, color: "var(--color-text-muted)", marginBottom: 4 }}>
                        <strong style={{ color: "var(--color-text-dim)" }}>Subject:</strong> {log.subject}
                      </p>
                    )}
                    <div style={{ display: "flex", gap: 14, fontSize: 12, color: "var(--color-text-dim)", flexWrap: "wrap" }}>
                      {log.recruiter && (
