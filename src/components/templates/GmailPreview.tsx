...
export function GmailPreview({
  ...
  const renderedSubject = useMemo(() => interpolate(subject) || "(no subject)", [subject]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="gmail-chrome bg-white rounded-xl overflow-hidden shadow-2xl border border-white/20 ring-1 ring-black/5 flex-1 flex flex-col">
        {/* Gmail top bar */}
        <div className="bg-[#f6f8fc] px-4 py-2.5 border-b border-[#e0e0e0] flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-baseline gap-1.5">
                <span className="text-[13.5px] font-semibold text-[#202124]">{senderName}</span>
                <span className="text-[11px] text-[#5f6368]">&lt;{senderEmail}&gt;</span>
              </div>
          ...
          <div className="px-5 pb-4 bg-white shrink-0">
            <div className="inline-flex items-center gap-3 px-3 py-2 border border-[#e0e0e0] rounded-lg text-[#202124] hover:bg-gray-50 transition-colors cursor-pointer">
              <div className="w-8 h-8 rounded-md bg-[#ea4335] flex items-center justify-center shrink-0">
                <span className="text-white text-[9px] font-bold tracking-wider">PDF</span>
              </div>
              <div className="pr-4">
        ...
        <div className="bg-[#f6f8fc] border-t border-[#e0e0e0] px-5 py-2.5 flex gap-4 shrink-0">
          {["Reply", "Reply all", "Forward"].map((a) => (
            <span key={a} className="text-[#5f6368] text-xs font-medium cursor-not-allowed hover:text-[#202124] transition-colors">{a}</span>
          ))}
        </div>
      </div>

      <p className="text-[11px] text-muted-foreground text-center mt-3 shrink-0">
        Preview uses sample data · {"{{name}}"} → Sarah Chen
      </p>
    </div>
  );
}
