...
const ENCRYPTED_MODELS: Record<string, string[]> = {
  ...
  Recruiter: ["name", "company", "role", "email", "linkedin", "location", "notes"],
  Resume: ["originalName"],
  Campaign: ["name"],
};
      ...
      async $allOperations({ model, operation, args, query }: any) {
        ...
        if (fieldsToEncrypt && args.data) {
          if (Array.isArray(args.data)) {
            args.data = args.data.map((item: any) => encryptObject(item, fieldsToEncrypt));
          } else {
...
function deepDecrypt(obj: any, modelName: string): any {
  ...
  if (Array.isArray(obj)) return obj.map((item) => deepDecrypt(item, modelName));
